...
const DoctorLoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      try {
        // Add userType to login method for doctor-specific login
        await login(username.toLowerCase(), password, 'doctor');
      } catch (err) {
  ...
  return (
    <div className="max-w-md mx-auto bg-white p-8 rounded-lg shadow-md mt-8">
      <h1 className="text-2xl font-bold text-red-600 mb-6 text-center">Doctor Login</h1>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex items-center justify-between mt-6">
          ...
          <Link href="/register/doctor" className="text-sm text-red-600 hover:underline">
            Register as Doctor
          </Link>
        </div>
